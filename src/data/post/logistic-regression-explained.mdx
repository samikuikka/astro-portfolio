...
Logistic regression is widely used for ninary classification.
...
And if we have N data points, each with feature vector <InlineLatex formula="x_n" /> and label <InlineLatex formula="y_n" />, we can write the likehood functions as:
...
Then we can get the sigmoid function as
...
Thus the rule for updating the weights is:
